...
def Run(subsample, branchsel, modules, postfix, json, isMC):
    if isMC:
        p = PostProcessor('.', subsample, None, branchsel, noOut=True, modules=modules, provenance=False, postfix=postfix.replace('.root', '_'+str(n)+'.root'), compression=0)
    else:
        p = PostProcessor('.', subsample, None, branchsel, noOut=True, modules=modules, provenance=False, jsonInput=json, postfix=postfix.replace('.root', '_'+str(n)+'.root'), compression=0)
    p.run()
...
parser.add_argument('-I', '--indir',  dest='indir',   action='store', type=str, default='.')
...
for entry in filelist:
        if not entry.startswith("#"): 
                ...
                infiles.append(args.indir+'/'+filepath[-41:].replace('.root', '_recal.root'))
