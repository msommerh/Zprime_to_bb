...
echo "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
echo
echo "combine ${inputfile} -M FitDiagnostics --setParameters ${set_params1} --toysFile higgsCombineTest.GenerateOnly.mH${mass}.${seed}.root -s ${seed} -t ${ntoys} --rMin ${rmin} --rMax ${rmax} --freezeParameters ${freeze_params} --cminDefaultMinimizerStrategy=0 --toysFrequentist --expectSignal ${expectSignal} --robustFit=1"
combine ${inputfile} -M FitDiagnostics --setParameters ${set_params1} --toysFile higgsCombineTest.GenerateOnly.mH${mass}.${seed}.root -s ${seed} -t ${ntoys} --rMin $rmin --rMax $rmax --freezeParameters ${freeze_params} --cminDefaultMinimizerStrategy=0 --toysFrequentist --expectSignal $expectSignal --robustFit=1
echo
echo "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
echo
