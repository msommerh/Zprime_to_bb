...
if [[ $combineYears -ne 1 ]]; then
    ...
    do
        ...
        if test -f "$card2"; then
                ...
                echo "output card: datacards/${btagging}/combined/bb_bq_combined${card1:2:$n}"
                #combineCards.py bb="$card1" bq="$card2" mumu="$card3" > "combined/combined${card1:2:$n}"
                combineCards.py bb="$card1" bq="$card2" > "combined/bb_bq_combined${card1:2:$n}"
            #else
    ...
    for card_2016_bb in bb_2016_M*.txt; do
        ...
        if [[ $ncards -eq 6 ]]; then
            ...
            echo "3) datacards/${btagging}/${card_2017_bb}"
            echo "4) datacards/${btagging}/${card_2017_bq}"
            #echo "6) datacards/${btagging}/${card_2017_mumu}"
            echo "5) datacards/${btagging}/${card_2018_bb}"
            echo "6) datacards/${btagging}/${card_2018_bq}"
            #echo "9) datacards/${btagging}/${card_2018_mumu}"
            echo "output datacard: datacards/${btagging}/combined/bb_bq_combined_run2${suffix}"
            ...
            combineCards.py bb_2016="$card_2016_bb" bq_2016="$card_2016_bq" bb_2017="$card_2017_bb" bq_2017="$card_2017_bq" bb_2018="$card_2018_bb" bq_2018="$card_2018_bq" > "combined/bb_bq_combined_run2${suffix}"
        else
