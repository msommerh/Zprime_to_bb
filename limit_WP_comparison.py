...
BTAGGING_WPS= ["loose", "medium", "tight"]
COLORS = {"loose": 2, "medium": 8, "tight": 4}
...
theoryLabel = {'B3' : "HVT model B", 'A1' : "HVT model A", "SSM" : "SSM Z'"}
...
def limit():
    ...
    for BTAGGING in BTAGGING_WPS: 
        ...
        for i, m in enumerate(mass_dict[BTAGGING]):
            ...
            Exp0s[BTAGGING].SetPoint(n, m, val_dict[BTAGGING][m][3]*multF*acc_factor)
    ...
    Exp0s[BTAGGING_WPS[0]].GetXaxis().SetTitleSize(Exp0s[BTAGGING_WPS[0]].GetXaxis().GetTitleSize()*1.25)
    Exp0s[BTAGGING_WPS[0]].GetXaxis().SetNoExponent(True)
    Exp0s[BTAGGING_WPS[0]].GetXaxis().SetMoreLogLabels(True)
    Exp0s[BTAGGING_WPS[0]].GetYaxis().SetTitle("#sigma("+particleP+") #bf{#it{#Beta}}("+particleP+" #rightarrow "+particle+"){} (fb)".format(" #times #Alpha" if INCLUDEACC else ""))
    ...
    c1.Print("combine/plotsLimit/ExclusionLimits/"+YEAR+"_WP_comparison.png")
    c1.Print("combine/plotsLimit/ExclusionLimits/"+YEAR+"_WP_comparison.pdf")
