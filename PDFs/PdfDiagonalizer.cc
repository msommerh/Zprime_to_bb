...
PdfDiagonalizer::PdfDiagonalizer(const char *name, RooWorkspace *w, RooFitResult &result, const char *prefix) :
    name_(name),
    parameters_(result.floatParsFinal())
{
    ...
    for (int i = 0; i < n; ++i) {
        snprintf(buff,sizeof(buff),"%s%s_eig%d[-5,5]", prefix, name, i);
    ...
    for (int i = 0; i < n; ++i) {   
        RooArgList coeffs;
        for (int j = 0; j < n; ++j) {
            snprintf(buff,sizeof(buff),"%s%s_eigCoeff_%d_%d[%g]", prefix, name, i, j, vectors(i,j)*sqrt(values(j)));
            coeffs.add(*w->factory(buff)); 
        }
        snprintf(buff,sizeof(buff),"%s%s_eigBase_%d[%g]", prefix, name, i, (dynamic_cast<RooAbsReal*>(parameters_.at(i)))->getVal());
        coeffs.add(*w->factory(buff)); 
        snprintf(buff,sizeof(buff),"%s%s_eigLin_%d", prefix, name, i);
